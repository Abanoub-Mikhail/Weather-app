...
async function getWeather(country="alex") {
    res = await fetch(`https://api.weatherapi.com/v1/forecast.json?key=f71c317b9b694ef8b88141142232912&q=${country}&days=3`);
    if (res.status == 400) {
...
function displayNextdayWeather (){
    for( let i=0; i<nextDayDate.length;i++){
        ...
        nextDayMonth[i].innerHTML = `${new Date(data.forecast.forecastday[i+1].date).getDay()} ${monthName[new Date(data.forecast.forecastday[i+1].date).getMonth()]}`;
